...
在上面的分支中，你还可以加上 CMAKE_CXX_FLAGS 的设置，例如给 Debug 加上 `-g -O2`，给 Release 加上 `-O0`，等等。这些都是题中应有之义。但是考虑到不同的编译器：clang, gcc, llvm, msvc, mccv clang-cl, ...，这样的分支也并不是那么容易编写。即使你终于写定了，也难以维护。即使你努力维护了，下一个项目可能需求根本不同，这些编译选项还得重新设计编写一套。
