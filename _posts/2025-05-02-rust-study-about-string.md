...
说来这也不难理解，复合逻辑。只不过 `&*` 着实难看。当然，觉得难看还是因为 `C/C++` 积习难改，这么一想，我也就心平气和了。
...
当 RR 的优点是有利于初学者学习。因为它的 AI 补全功能能够为你提示常见的补全片段，所以本文中提到的各种相互转换在 RR 中常常无需记忆可以无脑补全。但这个补全也不完全彻底可靠，因为有时候会产生复杂调用转换，而实际上可能只需要 `&*` 即可（只是一个特例，其他情形也有）。对于这个问题，本文就有用了。或者，你可以通过深度挖掘 Rust 程序设计教程以及相关的教科书来做到完全的语法概念理解，也就能发现那些冗余转换，从而给出正确的简化形式。
