...
> 毫无疑问，上面的示例不算太恰当，但它将能匹配 "cde" 这样的字符串。它的不恰当在于表达式中的前置断言其实几乎毫无意义。
>
> 但这个例子的意图只是为了解释 `(?!re)re` 这样的否定前瞻断言的格式中所包含的两个 `re` 子表达式。
>
> 一个类似的但有意义的正则式如 `^(?=<)<[^>]+>\w+` 其中的肯定前瞻断言确认了像 `<b>bold</b>` 这样的字符串蒋能被匹配。
>
> 结合 backref 的话，一个可能匹配成对 html 标记的正则式大约像这样：`(?=<)<([^]>+)>.*</\1>`，其中 `\1` 要求与前面的捕获组 `([^>]+)` 相同，所以它将会匹配 `<b>bold<i>italic</i></b>` 这样的成对的 html 标记。
